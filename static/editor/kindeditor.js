...
_extend(KUploadButton, {
	...
	submit : function() {
		...
		//因为submit有问题，所以我将submit事件提取了处理，单独进行ajax，设置的参数都是必需的
		if(self.form.attr('action') == QINIU_UPLOAD_URL){
			console.log($(self.form).find('input[name="token"]').val());
			...
			$.ajax({
				...
				success:function(resp){
					console.log(resp);
					self.options.afterUpload.call(self, resp);
				}
			})
			return self;
		}
...
KindEditor.plugin('image', function (K) {
	var self = this, name = 'image',
		...
		uploadJson = K.undef(self.uploadJson, 'http://upload.qiniup.com') //七牛云上传url华南区
		imageTabIndex = K.undef(self.imageTabIndex, 0),
		imgPath = self.pluginsPath + 'image/images/',
	...
	self.plugin.imageDialog = function (options) {
		...
		var uploadbutton = K.uploadbutton({
			...
			afterUpload: function (data) {
				...
				if (data.error === 0) {
					...
					file_key = K('[name="key"]', div).val()
					console.log(IMGURL + file_key);
					self.insertHtml('<img src="'+ IMGURL + file_key +'" />').hideDialog().focus();
				}
			},
			afterError: function (html) {
				dialog.hideLoading();
		...
		uploadbutton.fileBox.change(function (e) {
			...
			$.ajax({
				...
				success: function (resp) {
					// console.log(resp);
					qiniuToken = resp.upToken;
					K('[name="token"]', div).val(resp.upToken);
					K('[name="key"]', div).val(resp.upKey);
